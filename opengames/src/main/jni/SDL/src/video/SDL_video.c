...
SDL_SetWindowGammaRamp(SDL_Window * window, const Uint16 * red,
                                            ...
                                            const Uint16 * blue)
{
	LOGI("SDL_SetWindowGammaRamp 1");
    CHECK_WINDOW_MAGIC(window, -1);
    LOGI("SDL_SetWindowGammaRamp 2");
    if (!_this->SetWindowGammaRamp) {
        SDL_Unsupported();
        return -1;
    }
    LOGI("SDL_SetWindowGammaRamp 3");
    if (!window->gamma) {
        if (SDL_GetWindowGammaRamp(window, NULL, NULL, NULL) < 0) {
            return -1;
        }
    }
    LOGI("SDL_SetWindowGammaRamp 4");
    if (red) {
    ...
    LOGI("SDL_video input focus = %d",(window->flags & SDL_WINDOW_INPUT_FOCUS));
