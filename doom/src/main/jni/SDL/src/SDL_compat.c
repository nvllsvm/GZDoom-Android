...
SDL_ListModes(const SDL_PixelFormat * format, Uint32 flags)
{
	...
	//Both these needed for Wolf to work
	nmodes = 2;
	modes = SDL_realloc(modes, (nmodes + 2) * sizeof(*modes));
	modes[0] = (SDL_Rect *) SDL_malloc(sizeof(SDL_Rect));
	modes[0]->x = 0;
	modes[0]->y = 0;
	//modes[0]->w = Android_ScreenWidth;
	//modes[0]->h = Android_ScreenHeight;
	modes[0]->w = 640;
	modes[0]->h = 400; //CHANGED from 400 on 07/03/14

	modes[1] = (SDL_Rect *) SDL_malloc(sizeof(SDL_Rect));
	modes[1]->x = 0;
	modes[1]->y = 0;
	modes[1]->w = 320;
	modes[1]->h = 240;
